...
pub async fn get_all_flight_plans(
    pool: web::Data<Pool<ConnectionManager<PgConnection>>>
) -> impl Responder {
    debug!("Get fligths...");
    match database::get_all_flight_plans(pool) {
        ...
        Err(e) => {
            return HttpResponse::NoContent().body(format!("There are no flight plans filed with this system {}", e.to_string()));
        }
    }
}
        ...
        Ok(flight_plan) => {
            match flight_plan {
                Some(flight_plan_from_db) =>
                    return HttpResponse::Ok().content_type("application/json").json(flight_plan_from_db),
                None =>
                    return HttpResponse::NotFound().body(format!("There is not any flight plan with id {}", flight_plan_id))
            }
        }
        Err(e) =>
            return HttpResponse::NotFound().body(format!("There is not any flight plan with id {} with error {}", flight_plan_id, e))
    };
}
